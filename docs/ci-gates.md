...
- **閾値は「CIでの実測値」を1桁切り捨ててpinする** (`vitest.config.ts` の `thresholds` が正)。
  ...
  | 指標 | pin (`vitest.config.ts`) | pin根拠のCI実測 (run 31369760768 / 2026-08-10) | 同コードのローカル実測 (macOS) |
  |---|---|---|---|
  | statements | 28.1 | 28.11% (869/3091) | 28.11% (869/3091) |
  | branches | **29.4** | 29.49% (566/1919) | 29.65% (569/1919) |
  | functions | 31.4 | 31.42% (154/490) | 31.42% (154/490) |
  | lines | 28.0 | 28.03% (799/2850) | 28.03% (799/2850) |

  **branchesだけローカルが3本多い** (分母は同じ1919) のが上記のenv依存分岐。**pinはCI側の29.49を1桁切り捨てた29.4**。

  **これは「28% で十分」という主張ではなくラチェット**。下がったら落ちる。テストを足したら上げる。
...
現状の空白 (数値の裏にあるもの): `app/api/**/route.ts` 37本のうち直接テストがあるのは3本
(`api/cron/billing` / `api/user/line-link` / `api/user/line-link-liff`)。cron 4本のうち課金だけ単体テストが
入り (`__tests__/billing-cron-action.test.ts`)、**リマインド・農家通知・ログ掃除は単体テスト0件**。
課金cronもE2E側は `CRON_SECRET` 未注入でskipのため、**実ストアを通した裏取りはまだ無い**
(手順は `docs/subscription-test-plan.md` のレーンA)。
