...
- 最終更新: 2026-08-11 JST / elxea-developer (特商法ページに定期便の契約条件を追加し、未確定値5件を登録)
...
**13件のうち7件が解決済み。未確定は6件** — `subscription.firstDeliveryDate` (初回お届け日) と、
2026-08-11に追加した定期便の契約条件のうちShopifyの設定・実測を待つ5件。
...
1. **Shopifyに定期便の締日と発送スケジュールを設定するか** (初回お届け日の公開ブロッカー。
   ...
   使える決済手段・課金タイミング・解約の受付期限・マイページで変更できる項目** の4点も
   Shopify側の確定が要る (特商法ページの公開ブロッカー)
...
| 10 | `tokushoho.subscriptionFirstChargeTiming` | 特商法ページ IV-4 | 初回課金のタイミング | Setaka / Shopify設定 | Shopify selling planのbilling policy。特定商取引法 第11条2号 | 未確定 (公開ブロッカー) |
| 11 | `tokushoho.subscriptionRecurringChargeTiming` | 特商法ページ IV-4 | 2回目以降の課金のタイミング | Setaka / Shopify設定 | 同上 | 未確定 (公開ブロッカー) |
| 12 | `tokushoho.subscriptionCancelCutoff` | 特商法ページ IV-6 停止・解約の方法 | 解約・変更の受付期限 | Setaka / Shopify実測 | 解約の申出に期限があるなら表示が必須 (消費者庁ガイドライン別添9 2(2)⑥)。定期便LPのFAQは「発送日の3日前まで」と書いているが未実測なので確定扱いにしない | 未確定 (公開ブロッカー) |
| 13 | `tokushoho.subscriptionEditableFields` | 特商法ページ IV-9 お申し込み内容の変更 | マイページで変更できる項目 | Setaka / Shopify実測 | Shopify Customer Accountの実機確認。変更できない項目を「変更できる」と書くと不実表示 | 未確定 (公開ブロッカー) |
...
10. **解約受付期限のLPと特商法の不一致 (未解決)** — 定期便LPのFAQは「発送日の3日前までに
   マイページから停止・スキップ」と書いているが、この3日という値は未実測で根拠がない。
   特商法ページ側は仮値ガード (#12) に載せてあるので、実測して確定させるときに **LP側の
   FAQも同じ値に揃える**こと。
