...
| 外余白x | 16 | 16 | [OK] |
| content幅 | 343 | 343 | [OK] |
| PageHead kicker → 見出し | 8 | **8** | [OK] 共有 `ListPageHead` を `gap-2 lg:gap-3` に是正 (2026-08-09 / デザイン保守3b670c9d-064c-8113) |
| 見出し 行高 | 38 | 38.39 | [OK] |
...
## 6-2. 見出しレベル (C16-1で追加した観点)

QAバッチ3は「5本の対比表に見出しレベルの行が1つも無い」と指摘した。Figmaには
hタグ相当の情報が無いので、**Figma側は「フレームの入れ子」を、実装側は実DOMの
アウトライン実測**を突き合わせる。実測はPC 1440 / SP 375の両方で同じ結果。

| 画面 | Figmaのフレーム構造 | C16-1前 (実測) | C16-1後 (実測) | 判定 |
|---|---|---|---|---|
| People詳細 | STORIES帯は節見出しを**持つ** (テンプレ `7822:37213`) | `h1(44) → h2(20 節)x2 → h2(20 節「この人の記事」) → h2(14)x6 → h2(20 節)` | **`… → h2(20 節) → h3(14)x6 → h2(20 節)`** | [OK] C16-1で是正 |
| コレクション詳細 | カード帯はページ見出しの**兄弟** (節フレームが無い) | `h1(44) → h2(16)x12` | **同じ (変更なし)** | [OK] h1直下でh2が正 |
| h1の数 (両画面) | 1 | 1 | 1 | [OK] |
| レベルの飛び (両画面) | — | 0件 | 0件 | [OK] |
| タグと視覚サイズの逆転 (両画面) | — | 0件 | 0件 | [OK] |

**People詳細を是正した理由**: 節見出し「この人の記事」とその子カードが同じ`h2`だと
節境界がアウトライン上で消える。`ArticleCard` の `headingLevel` prop (既定 `h2`) に
`h3` を渡した。**体裁は1pxも変わらない**。PC / SPのフルページスクショは
**バイト単位で同一**を確認済 (実測: 同一ビルドを2回撮って差分0の決定性を確認した上で、
変更前後の差分も0)。

**コレクション詳細を変えなかった理由**: カード帯 (`ProductCard` → `CatalogCard`) は
ページ見出しの直下にあり、間に節見出しが無い。`h1 → h2` は単調で飛びも逆転も無いため
defectではない。加えて `CatalogCard` は共有部品 `components/catalog/catalog-list.tsx`
にあり、C16-1と並行して走る保守レーンが同ファイルを持っているため本レーンでは
触らない (確認事項に記載)。
...
1. leadの幅がFigma 304に対しcontent全幅 (PC) — **未解決 (残置)**
2. kicker → 見出しがFigma SP 8に対し12 — **解消2026-08-09**
3. 見出し → leadがFigma SP 8に対し12 — **解消2026-08-09**

いずれも `components/catalog/catalog-list.tsx` の `ListPageHead` が両BPで `gap-3` (12) 固定、
leadに幅制約を持たないことに由来する。**商品一覧 / お茶メニュー一覧 / コレクション一覧が
同じ部品を共有しており**、本レーンで直すと4画面の実測値が同時に動くため触らなかった
(専有範囲はPeople詳細・コレクション詳細の2画面とその専用部品)。

**後続: 2と3はデザイン保守タスク3b670c9d-064c-8113 (2026-08-09) が共有部品側で
`gap-2 lg:gap-3` に是正して閉じた** (SP 8 / PC 12をproduction buildで再計測)。
1 (leadの幅) はFigmaのleadがPCで304幅に収まる指定で、共有部品に幅制約を足すと
リスト系4画面すべてのleadが同時に狭くなる。SPには影響が無く画面ごとの正解が
分かれるため、**この1件はまだ残置**している (共有部品にmax-widthを持たせるか、
ページ側で渡すかの設計判断が要る)。
...
## 11. C16-1 (QAバッチ3の指摘修正) 時点の再計測

見出しレベルの是正内容は「6-2. 見出しレベル」を参照。ゲートの詳細は
`docs/fidelity/c10-1-fidelity.md` の「4-2. C16-1時点の再計測」に一本化してある。

| 検査 | 結果 |
|---|---|
| People詳細PC / SPのフルページスクショ | **変更前後でバイト同一** (同一ビルド2回撮りでも差分0の決定性を確認済) |
| コレクション詳細PC / SP | コード変更なし。スクショも**バイト同一** |
| 見出しレベルの飛び / 逆転 | 両画面0件 |
| HTTP / 横あふれ / console error | 200 / 0 / 0 |

**確認事項 (C16-1追加)**: コレクション詳細のカード見出し (`CatalogCard` の
`catalog-card-title`) はh2のまま据え置いた。(a) カード帯がページ見出しの直下で
節見出しが無いため `h1 → h2` は単調でdefectではない、(b) `CatalogCard` は
`components/catalog/catalog-list.tsx` にありC16-1と並行する保守レーンが同ファイルを
持っていて本レーンでは触れない、の2点が理由。**現状でよいかの確認**。
