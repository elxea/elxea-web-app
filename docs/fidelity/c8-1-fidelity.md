...
| # | 対象 | Figma (SoT) | 修正前の実装 | 修正後の実装 | 状態 |
|---|---|---|---|---|---|
| 1 | SP Hero内側3段の間隔 (`8109:46624`-`46627`) | 16 / 16 / 16 | 24 / 24 / 24 | **16 / 16 / 16** | [OK] |
| 2 | SP一報リストの行ピッチ (`8110:47189`) | 44 (行枠h44) | **60.0** (min-h44 + gap16) | **44.0** (行枠44 + gap 0) | [OK] |
| 3 | SP茶 (EC) のheadキッカー (`8109:46644`) | 出さない (見出しのみ) | "TEA LEAVES" を表示 | **SP非表示** (`display:none`) | [OK] |
| 4 | SPカテゴリの32px見出し (`8109:46629`) | 出さない (キッカーのみ) | 32px見出しを表示 | **SP非表示** (`sr-only` でa11yには残す) | [OK] |
| 5 | SPカテゴリのタイル数 (`8109:46631/46634/46637`) | 3 | 6 | **3** (PCは6のまま) | [OK] |
| 6 | 章切り キッカー→見出し (PC `8109:46593/46594` / SP `8109:46641/46642`) | 24 / 24 | 20 / 20 | **24 / 24** | [OK] |
| 7 | §4 SP行ピッチの記載 | 44 | 「実装44」と誤記 (実測60) | 実測値で書き直し | 誤記是正 |
| 8 | §7 SP term→valueの記載 | **4** (`8110:47116` h18 → value y22) | 「Figma 22」と誤記 (22はy座標) | Figmaも4 = 一致 | 誤記是正 |

### PCに回帰が無いことの数値 (修正前 → 修正後・PC 1440)

SPだけを動かす修正なのでPCは不変であるべき。修正で触れた項目すべてをPCで再実測した。

| PCの項目 | 修正前 | 修正後 | 判定 |
|---|---|---|---|
| Hero内側3段の間隔 | 24 / 24 / 24 | 24 / 24 / 24 | [OK] 不変 |
| 一報リストの行ピッチ / 行高 | 52.8 / 28.8 | 52.8 / 28.8 | [OK] 不変 |
| 一報リストの日付 (メタ) | 表示12px / lh 21 | 表示12px / lh 21 | [OK] 不変 |
| 茶 (EC) headキッカー→見出し | 12px | 12px | [OK] 不変 |
| カテゴリheadキッカー→見出し | 12px | 12px | [OK] 注24 |
| カテゴリのタイル数 / 列構成 | 6 / `416px x3` gap32 rowgap48 | 6 / `416px x3` gap32 rowgap48 | [OK] 不変 |
| 章切り キッカー→見出し | 20px | **24px** (Figma 24に一致) | [OK] 是正 |
...
| コンソール `error` / `pageerror` | 0 / 0 | 0 / 0 | [OK] |
...
| 節 | 高さ | 198 (47189) | 233 | **217** | [OK] Δ19注26 |
...
| 節 | 高さSP | 881.33 (46644) | 896.86 (修正前918.25) | [OK] Δ15.5注28 |
...
| 節 | 高さSP | 1037 (46629) | 1042.59 | [OK] Δ5.6注31 |
