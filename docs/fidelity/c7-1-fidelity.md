...
| Badge | **地色** | **secondary #d5d3c0** | **#ffc10d → 現 #d5d3c0 [解決 2026-08-09]** | **[DS案件]** 注4 |
...
| card | **罫線色** | **#888675** | **#858581 → 現 #888675 [解決 2026-08-09]** | **[DS案件]** 注6 |
| row | ラベル | 14 / 400 / muted | 14 / 400 / #595854 | [OK] |
| row | 値PC | 16 / 右寄せ | 16 / right | [OK] |
| row | 値SP | 14 / 右寄せ | 14 / right | [OK] |
| row | **値の色** | **#464748** | **#5d5e61 → 現 #464748 [解決 2026-08-09]** | **[DS案件]** 注6 |
| divider | 高さ / 幅 | 1 / 全幅 | 1 / PC 1262・SP 316 (= card内幅) | [OK] |
| divider | 色 | #888675 | #858581 → 現 #888675 [解決 2026-08-09] | [DS案件] 注6 |
| row間 | 溝 | 16 | 16 | [OK] |
| 日時表記 | 書式 | `2026年8月10日（日）14:00–17:00` | `2026年8月10日(月) 14:00–17:00` | [仕様] 注7 / 注16 |
...
注16 (C6-1Rで訂正): **この行の「実装 (実測)」は当時まだ実測値ではなかった**。見本データ
`seed-event-1` が `endDate` を持っておらず、`formatEventSchedule()` の**同日レンジ分岐を
一度も通っていなかった**ため、実際に描かれていたのは `2026年8月10日(月) 14:00` (開始のみ)。
表に書いてあった `14:00–17:00` はFigma側の値をそのまま写したものだった。

C6-1Rで見本に終了時刻 (`endDate` = 2026-08-10T08:00Z = 17:00 JST) を入れて分岐を通し、
Vercel Previewで実描画を計測して**上表の値が実測値になった**:

- `/ja/events/seed-event-1` の実測描画 = **`2026年8月10日(月) 14:00–17:00`**
- 一覧 `/ja/events` のカードは `2026年8月10日` (終了日を併記しない)

あわせて分岐を単体テストで固定した (`__tests__/format-event-schedule.test.ts` / 10件)。
「データが無くて分岐が死んでいる」のと「実装が壊れている」のを、以後は表を見ずに切り分けられる。

一覧カードは日付だけを出す面なので、終了日が**同日のときは併記しない**ようにした
(`isSameEventDay()`)。入れないと同日開催が「2026年8月10日 — 2026年8月10日」と
同じ日付を2回描く。
...
| CTA | **文字色** | **#f9f8f4** | **#ffffff** → 現 #f9f8f4 [解決 2026-08-09] | **[DS案件]** 注8 |
...
| 詳細リンク | 角丸 | 8 | 8 | [OK] |
| 詳細リンク | 罫線 | 1px #888675 | 1px #858581 → 現 **#888675** [解決2026-08-09 / 実測] | [OK] 注12 |
...
未指定だと本文色 #5d5e61で描かれていた (実測で検出)。当レーンではこのページ側で
`border-border` を足して回避した。
**[更新2026-08-09 / 罫線色の総点検TASK ID-7561]** その後、原因がDS側にあること
(`components/ui/button.tsx` の `outline` variantが `dark:border-input` しか持たず、
lightでは色が無い) を突き止め、**DS側に `border-input` を明示**して直した。
これに伴い当ページのローカル回避 (`className` の `border-border`) は不要になったため
**撤去した** (`components/events/event-detail.tsx`)。罫線色はDSが持つ。
残っていた差 (#888675 vs #858581) はトークン側のドリフトで、DSトークン整合タスクで
`border` = #888675に是正済み。実測 (Chromium / canvas `getImageData`) で
1px **#888675** を確認。
...
| 中のCTA | 高さ / 幅 | 43 / 358 | **43 / 358** (SP390時) | [OK] 注15 |
...
注15 (C6-1Rで追記): **CTA幅358はSP390限定の値**。バーは `inset-x-0` + `px-4`、
中のCTAは `w-full` なので幅 = **viewport幅 - 32** で決まる固定値ではない。
- SP390 (Figmaのフレーム幅・上表の計測条件) → 390 - 32 = **358** ← Figma一致
- SP375 (iPhone SE系) → 375 - 32 = **343**

つまり358は「Figmaと一致した」ではなく「Figmaと同じ390幅で測ったから一致した」。
実装側は幅を固定していないので、他のSP幅では別の値になるのが正しい挙動 (Figmaが
390フレームで358と描いているのと同じ設計)。上表の他のSP実測値も同様にSP390条件。
