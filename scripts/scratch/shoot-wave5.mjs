...
const pages = [
  ["farmers-list", "/ja/farmers"],
  ["farmer-detail", "/ja/farmers/yamada-farm"],
  ["events-list", "/ja/events"],
  ["event-detail", "/ja/events/beginners-tea-workshop"],
];
...
for (const [vp, w, h] of viewports) {
  ...
  for (const [name, path] of pages) {
    ...
    const out = `/tmp/wave5-${name}-${vp}.png`;
