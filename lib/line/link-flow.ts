 ...
 * 詳細はサーバログにだけ残す。
 *
 * ## なぜ `conflict` を `error` から分けるのか（M-1 / J-4）
 *
 * 「このメールアドレスには既に別の LINE が連携済み」は**恒久的な衝突**で、時間を
 * おいても直らない。ところが従来は 409 も 500 も同じ `error` に潰され、画面には
 * 「時間をおいてもう一度お試しください」と出ていた。つまり**永久に成功しない
 * 再試行を、成功しうるかのように案内していた**。
 *
